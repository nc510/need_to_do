...
class StarWrongPaperConfigAdmin(admin.ModelAdmin):
    """错题组卷额度（单例）：免费用户每日免费组卷次数 + 两种身份的单次免卡题量上限。"""
    ...
    fieldsets = (
        ...
        ('单次组卷题量上限（不使用组卷卡时）', {
            'fields': ('free_max_questions', 'member_max_questions'),
            'description': (
                '分别设置免费用户与高级功能（会员）用户<b>不使用组卷卡</b>时，单次组卷最多可选的题数（最小 1）。<br>'
                '超过对应上限就必须使用组卷卡，<b>用卡当次不限题量</b>。<br>'
                '举例：免费用户填 20、高级用户填 30，则免费用户一次最多免卡选 20 题，'
                '高级用户一次最多免卡选 30 题，更多题量两者都需用卡。'
            ),
        }),
    )
